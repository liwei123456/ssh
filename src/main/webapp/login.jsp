...
<head>
<title>登录页面</title>
<link href="<%=basePath%>resources/user/login/css/login.css" rel="stylesheet" type="text/css"/>
...
<script type="text/javascript">
	...
	function login(){
		...
		$.ajax({
			url:"<%=basePath%>user/userController!login.action",
			data:$("#form").serialize(),
			dataType:"json",
			type:"post",
			success:function(data){
				...
				if(code==1){
					location.href="<%=basePath%>user/userController!main.action";
				}else if(code==2){
					alert("用户名已存在！");
				}else if(code==3){
					alert("密码错误！");
			...
			<div style="height: 50px; line-height: 50px; margin-top: 30px; border-top-color: rgb(231, 231, 231); border-top-width: 1px; border-top-style: solid;">
				<p style="margin: 0px 35px 20px 45px;">
					...
					<span style="float: right;">
						...
						<a id="submitEnter" style="background: rgb(0, 142, 173); padding: 7px 10px; border-radius: 4px; border: 1px solid rgb(26, 117, 152); border-image: none; color: rgb(255, 255, 255); font-weight: bold; cursor: pointer;"
						onclick="login()">登录</a>
					</span>
				</p>
			</div>
